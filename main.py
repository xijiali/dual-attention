...
def get_data(source_data_dir, target_data_dir, height, width, batch_size, re=0, workers=8):

    dataset = DA(source_data_dir, target_data_dir)
...
def main(args):
    ...
    query_loader, gallery_loader = get_data(args.source_data_dir, args.target_data_dir,
                                            args.height,
    ...
    if args.visualize:
        print("Visualize:")
        evaluator.visualize(query_loader, gallery_loader, dataset.query,
                           dataset.gallery, args.output_feature)
        return
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if (epoch+1) % args.evaluate_epochs == 0 or (epoch+1) == args.epochs:
            print('Test model:')
            evaluator = Evaluator(model)
            mAP=evaluator.evaluate(query_loader, gallery_loader, dataset.query,
                               dataset.gallery, args.output_feature)
            if mAP>best_mAP:
                save_checkpoint({
                    ...
                    'epoch': epoch + 1,
                }, fpath=osp.join(args.logs_dir, 'checkpoint_best_mAP.pth.tar'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--evaluate_epochs', type=int, default=20)
    ...
    parser.add_argument('--source-data-dir', type=str, metavar='PATH',
                        default='/home/xiaoxi.xjl/re_id_dataset/DukeMTMC-reID')
    parser.add_argument('--target-data-dir', type=str, metavar='PATH',
                        default='/home/xiaoxi.xjl/re_id_dataset/Market-1501-v15.09.15')
    parser.add_argument('--logs-dir', type=str, metavar='PATH',
                        default=osp.join(working_dir, 'logs-2019-0517-0957'))
