...
class ResNet(nn.Module):
    ...
    def __init__(self, depth, pretrained=True, cut_at_pooling=False,
        ...
        # attention subnetwork output [32,1,16,8]
        self.sub_network = nn.Sequential(
            nn.Conv2d(2048, self.middle_dim, 1),
            nn.BatchNorm2d(self.middle_dim),
            nn.ReLU(inplace=True),
            nn.Conv2d(self.middle_dim, 1, 1),
            nn.Sigmoid()
        )
    ...
    def forward(self, x, output_feature=None):
        for name, module in self.base._modules.items():
            if name == 'avgpool':
                ...
                x = module(x)
        # attention
        attention=self.sub_network(x)
        out_attention=attention
        b,c,h,w=x.size()
        attention=attention.expand(b,c,h,w)
