...
class TripletLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        dist_ap = torch.stack(dist_ap,dim=0)#xjl changed cat to stack
        dist_an = torch.stack(dist_an,dim=0)#xjl changed cat to stack
...
class FeatureMimickingLoss(nn.Module):
    ...
    To make the two graph matching mask closer
    """
    def __init__(self):
        super(FeatureMimickingLoss, self).__init__()
        self.cos_d=nn.CosineSimilarity(dim=1, eps=1e-6)
    ...
    def forward(self, f1,f2):
        ...
        Args:
            ...
            targets: ground truth labels with shape (num_classes)
        """
        loss=(1-self.cos_d(f1,f2)).mean(0).sum()
        return loss
